...
{#if type === 'desktop'}
	<PrismicLink
		class="relative block px-3 py-1 overflow-hidden text-base font-bold rounded group text-slate-900"
		{field}
...
{:else}
	<PrismicLink
		class="relative block px-3 overflow-hidden text-3xl font-bold rounded group text-slate-900"
		{field}
		on:click={onLinkClick}
		aria-current={isActive ? 'page' : undefined}
	>
