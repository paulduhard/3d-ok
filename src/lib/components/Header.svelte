...
<header class="relative top-0 z-50 mx-auto max-w-7xl md:sticky md:top-4">
    <nav>
        <div class="flex flex-col justify-between px-4 py-2 rounded-b-lg bg-slate-50 md:m-4 md:flex-row md:items-center md:rounded-xl">
            ...
            <!-- Mobile Nav  -->
             <ul
                ...
                <li>
                    <button aria-expanded={open} aria-label="Close Menu"
                    class="fixed block p-2 text-2xl right-4 top-3 text-slate-800 md:hidden" on:click={() =>(open=false)}>
                        <IconClose />
                    </button> 
                </li>
            ...
            <!-- Desktop Nav -->
             <ul class="relative z-50 flex-row items-center hidden gap-1 py-0 bg-transparent md:flex">
                {#each settings.data.nav_item as {label, link}}
                    <li>
