...
<ul on:mouseleave={onMouseLeave} class="grid border-b border-b-slate-100">
    {#each items as post, index (post.id + index)}
        <li
            ...
            class="opacity-0 content-list-item" use:onItemEnter={index}>
           <PrismicLink document={post} class="flex flex-col justify-between py-10 border-t border-t-slate-100 text-slate-200 md:flex-row">
            <div class="flex flex-col">
                <span class="font-bold etxt-3xl">{post.data.title}</span>
            ...
            <span class="flex items-center gap-2 ml-auto text-xl font-medium md:ml-0">
                {viewMoreText}
                <IconArrow />
            </span>
...
<!-- Hover element -->
<div
    class="absolute top-0 left-0 pointer-events-none hover-reveal -z-10 h-[320px] w-[220px] rounded-lg bg-cover bg-center opacity-100 transition-[background] duration-300"
