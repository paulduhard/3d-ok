	...
	<div class="w-full mx-auto max-w-7xl">
		<div class="grid min-h-[65vh] grid-cols-1 items-center md:grid-cols-2">
			...
			<div class="col-start-1 md:row-start-1">	
				<h1 class="mb-2 text-[clamp(3rem,20vmin,13rem)] font-extrabold leading-none tracking-tighter text-nowrap"
				aria-label={slice.primary.first_name + ' ' + slice.primary.last_name}>
					...
					<span class="block text-slate-300">
						...
						<span class="inline-block opacity-0 name-animation">{letter}</span>
						{/each}
					</span>
					{/if}
